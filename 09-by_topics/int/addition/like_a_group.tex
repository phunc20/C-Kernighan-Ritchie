...
Một cách tự nhiên, mình quy định $(\overbrace{0\cdots0}^{\textrm{32 bits}})_{2} = 0, (\overbrace{0\cdots01}^{\textrm{32 bits}})_{2} = 1, (\overbrace{0\cdots010}^{\textrm{32 bits}})_{2} = 2, (\overbrace{0\cdots011}^{\textrm{32 bits}})_{2} = 3$, etc. Cái này cho đến khi $(\overbrace{01\cdots1}^{\textrm{32 bits}})_{2} = 2^{31} - 1$. Mình tạm dừng ở đây là tại vì (\romannumeral 1) đến đây mình đã cho hết một nửa các con số 32-bit; (\romannumeral 2) mình còn chưa assign số âm nào hết.
...
Để tìm được \texttt{b} sao cho \texttt{a} đi thêm \texttt{b} bước theo chỉ kim đồng hồ tới $(\overbrace{10\cdots0}^{\textrm{32 bits}})_{2} = 0$, mình có thể
\begin{itemize}
